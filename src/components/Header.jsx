  ...
  return (
    ...
    >
      <nav className="container mx-auto px-6 py-4">
        ...
        {/* Mobile Navigation */}
        {isMobileMenuOpen && (
          <div className="md:hidden mt-4 py-4 border-t border-slate-700 animate-in slide-in-from-top">
            {navItems.map((item) => (
              <a
                key={item.label}
                href={item.href}
                onClick={(e) => handleNavClick(e, item.href)}
                className="block py-2 text-slate-300 hover:text-emerald-500 transition-colors duration-200"
              >
                {item.label}
              </a>
            ))}
